...
def init(job):
    ...
    if service.model.data.deviceOs != 'custom_ipxe':
        ...
        os_actor.serviceCreate(service.model.name, args={
                               'node': service.model.name, 'sshkey': service.model.data.sshkey})
...
def install(job):
    ...
    if packetclient.model.data.token == "":
        raise j.exceptions.Input(
            message="token for packet.net cannot be empty", level=1, source="", tags="", msgpub="")

    hostname = service.model.data.deviceName
    # hostname = service.model.name

    project_name = service.model.data.projectName
    project_ids = [project.id for project in client.list_projects()
    ...
    project_devices = {
        device.hostname: device for device in client.list_devices(project_id)}
    if hostname not in project_devices:
        plan_type = service.model.data.planType
        plan_ids = [plan.id for plan in client.list_plans()
        ...
        if operating_system_name != 'custom_ipxe':
            ...
            if not operating_systems:
                raise RuntimeError(
                    'No operating_systems found with name %s' % operating_system_name)
    ...
    ip = [netinfo['address'] for netinfo in device.ip_addresses if netinfo['public']
          and netinfo['address_family'] == 4]
...
def uninstall(job):
    ...
    if device_id is None or device_id == '':
        raise j.exceptions.NotFound(
            "device id not know, node probably not installed")
